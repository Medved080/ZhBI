...
-- Справочник подтипов элементов ЖБИ по новому стандарту имён слоёв —
-- сознательно НЕ зашит в код разбора (scripts/layer_naming.py принимает
-- его параметром). Сравнение при разборе имени слоя — без учёта регистра
-- (см. layer_naming._ci_lookup), поэтому регистр здесь — просто то, что
-- будет показано в UI, а не что-то, что нужно дублировать под каждый
-- возможный вариант написания в файле.
--
-- СПРАВОЧНИК ПРИНАДЛЕЖИТ ОБЪЕКТУ, а не системе (2026-08-21, решение
-- пользователя). Подтип у этого заказчика — это, как правило, ОТМЕТКА
-- перекрытия («на отм. +15.000»), то есть свойство конкретного здания:
-- у соседнего дома другая высота этажа и, значит, полностью другой набор
-- отметок. Общий на всю систему справочник смешивал их в одну кучу — в
-- фильтрах и массовой правке Объекта-1 появлялись два десятка отметок
-- Объекта-2, которых в нём нет ни у одного изделия.
--
-- Новый объект заводится с ПУСТЫМ справочником и наполняется сам при
-- загрузке чертежа: подтип, которого ещё нет, добавляется разбором (см.
-- app/dxf_import.py). Поэтому здесь нет и стартового набора значений —
-- раньше он сидел прямо в схеме, и на любой свежей базе в справочнике
-- сразу лежали отметки конкретного дома заказчика. Накопленные строки
-- разводятся по объектам миграцией `_migrate_allowed_subtypes_to_object`
-- (app/db.py) — по тому, какой объект эти подтипы РЕАЛЬНО использует.
...
-- Обработки релиза (2026-08-04): что новая версия кода делает с уже
-- накопленными данными, чтобы они подходили этому коду. Выполняются САМИ
-- при первом старте после обновления — подключаться к серверу для этого не
-- нужно (требование пользователя: «добавляй все обработки в само обновление
-- и пусть они сами выполняются при старте»). Реестр — app/release_tasks.py,
-- здесь только учёт: что уже выполнено, чем закончилось и когда.
--
-- Учёт ПО ИМЕНИ, а не по номеру версии: версия отвечает на вопрос «до какого
-- релиза доведена база» (app_settings.db_release_version) и годится для
-- показа, но не годится для решения «выполнять или нет» — обработка,
-- добавленная задним числом в уже вышедшую версию, при сравнении номеров
-- была бы пропущена молча.
CREATE TABLE IF NOT EXISTS release_tasks (
    name TEXT PRIMARY KEY,                      -- стабильный идентификатор обработки
    version TEXT,                               -- релиз, в котором она появилась
    kind TEXT NOT NULL DEFAULT 'data',          -- data — при старте; cleanup — только по кнопке админа
...
CREATE TABLE IF NOT EXISTS object_levels (
    ...
    -- Явная высота этажа, мм (2026-09-02). NULL — неизвестна, тогда высота
    -- блока считается по отметке следующего этажа секции (см.
    -- app/block_geometry._level_height). Заполняет загрузка из PDF (обоих
    -- режимов) из таблицы этажей чертежа: техпространство секции 1 — 1,79м,
    -- а следующий этаж секции над ним стоит на 3,75м выше.
    height_mm REAL,
...
-- Внешние 3D-модели ОБЪЕКТА (благоустройство, позже фасады) — см.
...
-- группирует объекты и своих данных не имеет, как и везде в системе).
-- Одна строка на сохранённый слой; несколько слоёв на объект допускаются с
-- самого начала. Привязка к координатам объекта не доказана
-- (placement_mode='unreferenced' в первой версии) — anchor/offset лишь
-- ставят модель в разумное начальное положение и дают её подвинуть, а не
-- геопривязывают.
CREATE TABLE IF NOT EXISTS object_external_models (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    object_anchor_x_mm REAL NOT NULL,
    object_anchor_y_mm REAL NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_object_external_models_object
    ON object_external_models (object_id);
