...
("supplier_change_docs", "supplier_change_docs — Документ «Смена поставщика»", C_CONTR, S_CONTR, [
    ...
    ("from_contract_id", "INTEGER", "FK", "текущий контракт → contracts.id (RESTRICT)"),
    ("to_contract_id", "INTEGER", "FK", "новый контракт → contracts.id (RESTRICT)"),
    ("reason", "TEXT", "", "причина смены поставщика (сроки, качество, иное)"),
...
("supplier_change_items", "supplier_change_items — Позиции документа", C_CONTR, S_CONTR, [
    ...
    ("element_id", "INTEGER", "FK", "перенесённое изделие → elements.id (CASCADE)"),
    ("element_type", "TEXT", "", "тип изделия СНИМКОМ на момент переноса"),
    ("mark", "TEXT", "", "марка изделия СНИМКОМ на момент переноса"),
    ("status_at_move", "TEXT", "", "статус изделия на момент переноса (всегда ниже «Отгружен»)"),
]),
