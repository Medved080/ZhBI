...
("object_works", "object_works — Работа объекта МФР (вне блоков)", C_ELEM, S_ELEM, [
    ("id", "INTEGER", "PK", "идентификатор работы объекта"),
    ("object_id", "INTEGER", "FK", "объект → objects.id (CASCADE)"),
    ("work_type_id", "INTEGER", "FK", "операция → work_types.id (CASCADE); пара с object_id уникальна — наличие строки = операция в плане объекта (в т.ч. с неблочной единицей измерения)"),
    ("plan_start", "TEXT", "", "плановый срок начала, ГГГГ-ММ-ДД"),
    ("plan_end", "TEXT", "", "плановый срок окончания"),
    ("forecast_start", "TEXT", "", "прогнозный срок начала"),
    ("forecast_end", "TEXT", "", "прогнозный срок окончания"),
    ("note", "TEXT", "", "примечание к работе"),
    ("retired_at", "TEXT", "", "операцию сняли из плана объекта — мягкая пометка вместо удаления, факт остаётся адресуемым; NULL — в активном плане"),
    ("created_at", "TEXT", "", "момент включения операции в план объекта"),
    ("created_by", "INTEGER", "FK", "кто включил → users.id (SET NULL)"),
    ("updated_at", "TEXT", "", "момент последней правки сроков/примечания; входит в версию для сверки при сохранении"),
    ("updated_by", "INTEGER", "FK", "кто правил последним → users.id (SET NULL)"),
]),
("object_fact_reports", "object_fact_reports — Документ факта по работам объекта", C_ELEM, S_ELEM, [
    ("id", "INTEGER", "PK", "идентификатор документа"),
    ("object_id", "INTEGER", "FK", "объект → objects.id (CASCADE)"),
    ("report_date", "TEXT", "", "дата, на которую зафиксирован процент выполнения"),
    ("created_at", "TEXT", "", "момент создания"),
    ("created_by", "INTEGER", "FK", "кто создал → users.id (SET NULL)"),
    ("updated_at", "TEXT", "", "момент последней правки документа"),
    ("updated_by", "INTEGER", "FK", "кто правил последним → users.id (SET NULL)"),
]),
("object_fact_items", "object_fact_items — Процент по работе объекта в документе факта", C_ELEM, S_ELEM, [
    ("report_id", "INTEGER", "PK,FK", "документ → object_fact_reports.id (CASCADE)"),
    ("object_work_id", "INTEGER", "PK,FK", "работа объекта → object_works.id (CASCADE)"),
    ("percent", "INTEGER", "", "процент выполнения на дату документа, 0..100"),
]),
