...
#    по обеим граням), высота — по подписям отметок (с 2026-09-12 ярус
#    есть и в `pdf_rooms.FLOOR_PLANS`, детальный разбор читает лист 16).
...
def compute_facade_blocks(doc) -> dict:
    ...
    # Выход на кровлю секции 2 растр фасада не видит (см. `_PLAN_ROOF2_*`
    # выше) — его ярус, с 2026-09-12 присутствующий в `FLOOR_PLANS` для
    # детального разбора, здесь по-прежнему снимается с плана, не с фасада.
    floors = [p for p in pdf_rooms.FLOOR_PLANS
              if p.z0 >= 0 and p.floor != _PLAN_ROOF2_FLOOR]
    bands = [
        (p.floor, p.z0,
...
def _plan_rooms_canonical(doc, page_no: int, grid: dict) -> list:
    ...
    сетки — на листе 16 это было ровно (15030, 7630) по всем 45 подписям.
    С 2026-09-12 ту же поправку для такого листа даёт сама
    `_page_shift_correction` (подписи со всего листа, когда рядом с
    помещениями их нет) — остаток здесь стал нулевым, оставлен страховкой."""
    page = doc[page_no - 1]
