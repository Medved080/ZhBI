...
# Статус записи справочника «Проекты и объекты» (2026-09-07, расширен
# 2026-09-08 при импорте реестра заказчика с собственным «Статусом ОС» —
# у него было четыре значения против прежних трёх, и «Перспективный» с
# «Приостановлен» не сводились ни к одному честно: перспективный — это ещё
# не «в работе», а приостановленный — не «архивный» («приостановлен»
# бывает временно, архив — это «убрано из оборота» насовсем).
#
# perspective — рассматривается, работы ещё не идут;
# active      — в работе, показывается везде;
# suspended   — приостановлен: видна, но с меткой, как и завершённая;
...
#               архивные». ЕДИНСТВЕННЫЙ статус, скрытый по умолчанию —
#               остальные четыре остаются видимыми всегда.
#
# Набор значений держит КОД, а не CHECK в схеме: SQLite не принимает CHECK в
# ALTER TABLE ADD COLUMN — ровно как у objects.kind.
CATALOG_STATUSES = ("perspective", "active", "suspended", "completed", "archived")
...
class ObjectOut(AddressFields):
    ...
    # дерева и получать 404 у трёх объектов из четырёх.
    has_avatar: bool = False
    # id вложения-превью — только чтобы блок вложений отметил, какая из
    # приложенных картинок сейчас превью; саму картинку показывает has_avatar.
    avatar_attachment_id: Optional[int] = None
