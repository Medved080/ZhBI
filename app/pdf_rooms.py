...
# Тоньше — не стена, а линия на слое материала (контур кровли, край
# плиты: 40мм на слое монолита). Самая тонкая НАСТОЯЩАЯ фигура комплекта —
# перегородка коммуникационной шахты 50мм (легенда: одинарный
# металлический каркас с ГКЛВ, по одной на этаж) — порог ниже неё, не по
...
# Прямое уточнение пользователя (2026-09-02): «не должно быть
# периметральных стен, только конструкция, выделенная цветом на схеме».
# Первая версия (2026-09-02, утро) отбрасывала на кровле сегменты дальше
# 1м от помещений — уходил парапет по периметру, но оставались кладка
...
# Рампа въезда в паркинг — четвёртый блок подземного этажа (2026-09-02,
# схема пользователя); своих помещений на слое площадей у неё нет, блок
# строится по соседям (`pdf_facade_import._basement_boxes`).
_RAMP_SECTION = "Рампа"
...
_SPEC = [
    ...
    # Техническое пространство секции 1 — ярус до низа выхода на кровлю
    # (29400 по разрезу), хотя в экспликации листа 10 оно «H-1,79 м»:
    # попытка дать ему 1,79м (2026-09-02) оторвала выход на кровлю от
    # техэтажа в модели — пользователь велел вернуть; высота яруса здесь —
    # шаг до следующего яруса, не высота помещения.
...
def parse_walls_document(doc, rooms: list, on_progress=None) -> tuple:
    ...
    for plan in FLOOR_PLANS:
        ...
        # Верхние ярусы (см. `_STRUCTURE_ONLY_FLOORS`): остаётся только
        # конструкция, выделенная на листе цветом, — монолит; периметральные
        # стены, парапет, кладка вентшахт и площадок под оборудование
        # отбрасываются.
        if plan.floor in _STRUCTURE_ONLY_FLOORS:
            kept = [s for s in floor_segments if s.get("material") in _STRUCTURE_MATERIALS]
            dropped = len(floor_segments) - len(kept)
            if dropped:
                warnings.append(
                    f"Лист {plan.page} (этаж {plan.floor}): {dropped} фигур(-ы) слоёв "
                    "стен не монолит — периметр/парапет/кладка на этом ярусе не "
                    "стены (только выделенная цветом конструкция), отброшены")
            floor_segments = kept
        # Фигура тоньше самой тонкой стены легенды — линия, не стена (контур
        # кровли, край плиты — на слое монолита, 40мм): на «10 этаже
        # (секция 1)» такая линия 22,6м вдоль верхнего края здания в 9м от
        # надстройки становилась трёхметровой стеной по периметру (живой
        # запрос пользователя 2026-09-02, правило «только монолит» её не
        # ловило — слой-то монолитный). Правило общее для всех этажей: на
        # типовых этажах те же 40мм-линии монолита встречались по 2-3 штуки.
        thin = [s for s in floor_segments if s["thickness_mm"] < _MIN_WALL_THICKNESS_MM]
        if thin:
            warnings.append(
