...
def my_permissions(object_id: Optional[int] = Query(None),
    ...
    try:
        ...
        return {
            ...
            "object_kind": данные["object_kind"],
            # Разделы, которых на этом объекте НЕТ по его типу. Отдаются
            # отдельным списком, а не выводятся из нулевого уровня: клиент
            # пропускает администратора сервиса в обход уровней (требование
            # 2026-08-17), и тип объекта иначе обходился бы вместе с ними —
            # администратор видел бы на объекте ЖБИ рабочее место МФР.
            # Тип — не право, и обходить его нельзя.
            "not_applicable": [f["key"] for f in данные["features"] if f["not_applicable"]],
