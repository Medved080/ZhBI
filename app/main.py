...
def on_startup():
    ...
    # Внешние 3D-модели (благоустройство): подчистить только забытые
    # temp-файлы (упавшие между записью и atomic rename) — это безопасно
    # ВСЕГДА, .tmp-* не бывают ничьей строкой ни в одной базе.
    #
    # УБОРКА ФАЙЛОВ-СИРОТ (cleanup_orphan_model_files) ЗДЕСЬ НЕ ВЫЗЫВАЕТСЯ
    # — критическая ошибка нашлась и снята 2026-09-11 (живой отчёт
    # пользователя, 410 Gone на реальном сервере). Каталог
    # uploads/external_models/ ОДИН на диске для ЛЮБОЙ базы (не зависит от
    # ZHBI_DB_PATH), а "known" считался ТОЛЬКО по базе ЭТОГО процесса —
    # значит старт анон-копии удалял файлы боевых строк, и наоборот. При
    # --reload у пользователя (перезапуск на каждую правку .py) это било
    # по боевым данным при каждой моей правке кода. Безопасная версия —
    # ручная кнопка администратора, которая сверяет ОБЕ базы разом или
    # сравнивает с явно переданным списком; автоматической здесь быть не
    # должно, пока такой развязки нет.
    try:
        from app.external_model_storage import cleanup_orphan_temp_files
...
class BulkEditExportIn(BaseModel):
    ...
    element_ids: Optional[list[int]] = None
    # Выгрузить элементы ОДНОГО объекта (новый интерфейс: альтернатива снимку фильтра схемы и «всем объектам»). Сервер сам берёт те же
