...
ACTION_TITLES = {
    ...
    "contracting_bulk_edit": "Массовая правка контрактации (Excel)",
    ...
    "user_view3d": "Начальный ракурс 3D",
    "user_changelog_ack": "Ознакомился с «Что нового»",
    # Действия, которые до 2026-08-03 попадали в отчёт голыми кодами: отбор
    # действий — денилист (новое действие обязано попадать в отчёт само), но
    # русское название ему приходится дописывать сюда.
    "db_status_open": "Просмотр состояния БД",
    "db_status_table_view": "Просмотр таблицы БД",
    "fill_empty_scope": "Заполнение пустых «Объект» и «Проект»",
    ...
    # Справочники: удаление с заменой, перенос подчинённых, автоматическая
    # свёртка задвоенного (2026-08-05…06).
    "dictionary_delete": "Запись справочника удалена",
    "dictionary_merge": "Записи справочника объединены",
    "dictionary_fold": "Свёрнута задвоенная запись справочника",
    "mark_create": "Марка добавлена",
    "mark_rename": "Марка переименована",
    "element_contract_set": "Контракт изделия назначен вручную",
    # Документ «Смена поставщика» (2026-08-11): поэлементное событие и
    # сводка по документу. Подписи разные намеренно — в отчёте человека
    # интересует, сколько изделий он перевёл, а не сколько раз нажал «Записать».
    "supplier_change": "Изделие переведено на другой контракт",
    "supplier_change_doc": "Документ «Смена поставщика» записан",
    # Проведение документа (2026-08-11): черновик данные не меняет, поэтому
    # его создание и удаление — отдельные события от применения.
    "supplier_change_draft": "Черновик документа контрактации создан",
    "supplier_change_delete": "Черновик документа контрактации удалён",
    "supplier_change_post": "Документ контрактации проведён",
    "supplier_change_unpost": "Проведение документа контрактации отменено",
    "link_swap": "Обмен привязками изделий",
    "user_menu_prefs": "Настройка меню «Действия»",
    "user_min_label_px": "Порог показа подписей",
    # Режим «Зайти под пользователем» и работа с сеансами.
    "impersonate_start": "Вход под пользователем начат",
    "impersonate_end": "Вход под пользователем завершён",
    "session_revoked": "Сеанс прерван",
    "sessions_pruned": "Просроченные сеансы удалены",
    "password_changed": "Пароль изменён самим пользователем",
    # Системные события старта и обновления. Раньше показывались голыми
    # кодами: подпись обязана быть у КАЖДОГО действия, иначе журнал читается
    # как список ошибок (живой репорт 2026-08-06).
    "server_start": "Запуск сервиса",
    "schema_migration": "Миграция схемы БД",
    "release_task": "Обработка обновления выполнена",
    "release_task_failed": "Обработка обновления не выполнена",
...
ELEMENT_CHANGE_ACTIONS = (
    ...
    "element_comment",
    "planned_date",
    # Смена поставщика меняет контракт изделия и добавляет запись в его
    # историю статусов — это изменение реквизитов изделия, и в фильтре
    # «Изменения» ему место (2026-08-11).
    "supplier_change",
    "link_swap",
