...
# ACTION_TITLES и action_title переэкспортируются: отчёт остаётся тем
# местом, откуда их берут потребители (app/main.py и проверочные скрипты),
# а сам список живёт в реестре.
from app.activity_actions import (  # noqa: F401
    ACTION_CATEGORIES, ACTION_TITLES, CATEGORY_DENIED, CATEGORY_ERROR, action_title,
)
...
# Человеческие названия действий живут в ОДНОМ реестре с категориями
# (app/activity_actions.py, 2026-08-20). Раньше словарь стоял здесь, а
# категории не было вовсе; держать два списка одних и тех же кодов в
# разных файлах — верный способ разойтись на первом же новом действии.
# Неизвестное действие показывается КАК ЕСТЬ, а не прячется: набор
# действий растёт, и забытая подпись должна выглядеть некрасиво, а не
# превращаться в молча пропавшую работу (полноту стережёт
# scripts/check_activity_actions.py в pre-commit).
...
NON_CHANGE_ACTIONS = {
    ...
    "server_start", "schema_migration", "client_batch_truncated",
}
# Ошибки и отказы (2026-08-20) работой не являются по определению, и
# перечислять их поимённо здесь не нужно: они берутся ИЗ РЕЕСТРА по
# категории. Иначе каждый новый вид сбоя приходилось бы дописывать сюда
# руками — а забытый показывался бы прорабу как сделанная им работа.
NON_CHANGE_ACTIONS |= {
    code for code, category in ACTION_CATEGORIES.items()
    if category in (CATEGORY_ERROR, CATEGORY_DENIED)
}
