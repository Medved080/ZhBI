...
// Метка объекта — форма булавки, а не плоский кружок (живой запрос
// 2026-09-08: «серые точки объектов на сером фоне карты не видны... можем
// поставить туда произвольную иконку?»). Кружок того же серого цвета, что и
// подложка (земля #f5f5f0, фон #eceff1), тонул в ней почти без следа;
// узнаваемый силуэт булавки виден даже нейтральным цветом. Своя SVG, а не
// внешняя картинка: контур нарисован дугами Безье от круга к острию — тот
// же общий приём, что у мини-карты формы объекта, где такую же булавку уже
// рисует штатный `ml.Marker` (там один перетаскиваемый маркер — DOM-элемент,
// на два-три сотни точек с кластеризацией не годится, там нужен GL-слой).
//
// SDF (`sdf: true` в addImage) — картинка ОДНОГО цвета, а перекрашивает её
// в цвет доли смонтированного `icon-color` уже на карте, по каждой точке
// свой: одна иконка на все статусы, а не N заготовленных цветных файлов.
const МЕТКА_ШИРИНА = 96, МЕТКА_ВЫСОТА = 128;
let меткаPromise = null;
function загрузитьМетку() {
  if (!меткаPromise) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${МЕТКА_ШИРИНА}" `
      + `height="${МЕТКА_ВЫСОТА}" viewBox="0 0 24 32">`
      + `<path fill-rule="evenodd" fill="#000" d="`
      + `M12,0 C5.373,0,0,5.373,0,12 C0,21,12,32,12,32 C12,32,24,21,24,12 C24,5.373,18.627,0,12,0 Z `
      + `M12,17 C9.239,17,7,14.761,7,12 C7,9.239,9.239,7,12,7 C14.761,7,17,9.239,17,12 C17,14.761,14.761,17,12,17 Z`
      + `"/></svg>`;
    const img = new Image(МЕТКА_ШИРИНА, МЕТКА_ВЫСОТА);
    меткаPromise = new Promise((resolve, reject) => {
      img.onload = () => resolve(img);
      img.onerror = () => reject(new Error("не удалось построить иконку метки"));
...
export async function renderProjectMap(контейнер, { onOpenObject, onEmptyCoords }) {
  ...
  // Регистрируется НА ЭТОТ конкретный экземпляр карты (при каждом открытии
  // экрана строится новый `карта`, старый уничтожается — см. openProjectMap
  // в app.js): картинка кэширована модулем и грузится один раз, а
  // addImage() вызывается заново для каждой карты, иначе метка не появится
  // при повторном открытии экрана.
  карта.addImage("метка-объекта", await загрузитьМетку(), { sdf: true });
  ...
  карта.addLayer({
    id: "объекты-точки", type: "symbol", source: "объекты",
    filter: ["!", ["has", "point_count"]],
    layout: {
      "icon-image": "метка-объекта",
      // Размер по числу элементов: большая стройка должна и выглядеть
      // крупнее, иначе карта врёт о масштабе работ. Множитель, а не
      // абсолютные пиксели — картинка сама 96×128.
      "icon-size": ["interpolate", ["linear"], ["get", "elements"],
        0, 0.2, 100, 0.24, 1000, 0.3, 10000, 0.4],
      // Остриё булавки — точно в координате объекта, как у обычного
      // маркера; кружок вместо этого центрировался бы НАД точкой.
      "icon-anchor": "bottom",
      "icon-allow-overlap": true,
    },
    paint: {
      "icon-color": ["get", "color"],
      // Белая обводка — тот же приём, что был у кружков: серая булавка
      // (объект без данных) не тонет в светлой подложке земли/фона.
