 ...
 * @param {'ground'|'facade'} [opts.kind] — только для метаданных/подписи в
 *   UI, на anchor по Z НЕ влияет (исправлено 2026-09-11, Docs/
 *   fbx-placement-repair-claude-prompt.md §3 — раньше 'ground' брал верх
 *   габарита, из-за чего высокие деревья/оборудование опускали площадку
 *   вниз на свою высоту). Anchor по Z — всегда нижняя точка габарита, для
 *   обоих видов одинаково, чисто технический (см. ниже).
...
export async function loadExternalModelFbx({ arrayBuffer, THREE, FBXLoader, kind = "ground", limits = {} }) {
  ...
  // Anchor по высоте — ВСЕГДА нижняя точка габарита, независимо от kind
  // (исправлено 2026-09-11, Docs/fbx-placement-repair-claude-prompt.md §3;
  // раньше 'ground' брал ВЕРХНЮЮ точку габарита — этой точкой оказывалась
  // верхушка самого высокого дерева/оборудования, и уровень площадки
  // проваливался вниз на разницу высоты дерева, у каждой модели по-своему).
  // Anchor — ЧИСТО технический (для точности Float32 при запекании
  // геометрии в локальные координаты, см. translateAnchor ниже) и НЕ несёт
  // смысла «уровень земли/пола объекта» — это НЕЛЬЗЯ вычислить из одного
  // bbox без независимого высотного ориентира. Ручная привязка высоты —
  // числовым полем offset_z_mm, либо переносом уже подтверждённой привязки
  // с другого файла той же сцены («Перенести эту привязку на» —
  // `transferPlacementZ` в coordinates.js уже даёт корректный общий tZ,
  // если оба файла заведомо из одного источника координат). НЕ мигрирует
  // уже загруженные модели — anchor хранится один раз при разборе, эта
  // правка касается только новых загрузок (source_anchor_z_mm существующих
  // строк не пересчитывается).
  const anchorZ = minZ;
