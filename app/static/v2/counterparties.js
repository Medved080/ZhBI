...
// Доработка 2026-09-22 (область «комплектовщик», Docs/v2-progress/picker.md):
// — правка записи идёт с `expected_version` (app/record_version.py): конфликт устаревших данных — диалог «Отмена / Показать актуальные / Сохранить поверх»
//   (patchChecked); при неизвестном исходе записи (обрыв связи) автоповтора нет, факт читается с сервера (createChecked / reconcileUpdate);
// — удаление контрагента, договора и спецификации — по плану последствий с выбором замен (delete-plan.js); контракт — свой пикер замены ниже;
// — на вкладке «Позиции» контракта видны «привязано» и «остаток», предупреждения о количестве ниже привязанного и об удалённых позициях с привязанными
//   изделиями (сервер такое не сохранит — страж покрытия);
// — переход из раздела «Контракты» (contracts-list.js) открывает нужный контракт через разовую ссылку sessionStorage `v2.cp.deeplink`.
...
export function mountCounterparties(container, ctx) {
  ...
  function contractArchiveHint(contract) {
    ...
    return {
      ...
      text: blocks
        ? `Перевести в архив нельзя: к контракту привязано изделий — ${linked}. Перенесите их на другой контракт этой же спецификации через «Удалить контракт» или снимите привязку в рабочем месте «Модель: схема 2D/3D» (карточка изделия → «Контрактация»).`
    ...
    try {
      ...
      if (plan.plan && plan.plan.needs_replacement) {
        ...
        if (!candidates.length) {
          await showInfoDialog(`К контракту привязаны изделия схемы, а заменить контракт нечем — под этой спецификацией нет другого контракта. ${consequences} Заведите контракт-замену по этой же спецификации или измените привязку изделий в рабочем месте «Модель: схема 2D/3D».`);
          return;
        }
  ...
  function renderContractExpandedTabContent(id) {
    ...
    body.innerHTML = `
      <p class="v2-muted">Сначала — элементы схемы, уже привязанные к контракту (плановую дату можно проставить прямо здесь), затем — незаполненные позиции контракта (ещё без привязки). Привязку одного изделия меняют в V2, в рабочем месте «Модель: схема 2D/3D» (карточка изделия → «Контрактация»); привязку группы — там же после выделения рамкой, в форме «По строкам: статус со своим контрактом…».</p>
