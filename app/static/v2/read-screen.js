...
export function mountReadScreen(el, { screen, structure, objectId, api, groupTitle, rights }) {
  ...
  function paintReport(sec, s, bodyEl) {
    ...
    const exportBtns = (sec.exports || []).map((x) => `<button type="button" class="v2-btn" data-export="${x}">Выгрузить в ${x.toUpperCase()}</button>`).join("");
    const printBtn = `<button type="button" class="v2-btn" id="rd-print">Печать</button>`;
    const helpBtn = sec.helpKey ? `<button type="button" class="v2-btn" id="rd-help">Справка</button>` : "";
    const exportsBar = `<div class="v2-bar v2-export-bar">${printBtn}${exportBtns}${helpBtn}<span class="v2-muted" id="rd-export-status" role="status" aria-live="polite"></span></div>`;
    ...
    bodyEl.querySelector("#rd-print")?.addEventListener("click", () => window.print());
