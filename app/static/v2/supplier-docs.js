  ...
  function docHtml() {
    ...
    return `<div class="v2-bar"><h3>${esc(KIND_TITLE[x.kind])} — ${x.id ? `№ ${esc(x.number)} от ${ruDate(x.date)}` : "новый документ"} <span class="v2-tag">${x.id ? (ro ? "Проведён" : "Черновик") : "Черновик (не сохранён)"}</span></h3><button type="button" class="v2-btn" data-a="back" ${S.busy ? "disabled" : ""}>← К списку</button></div>
      ...
      <div class="v2-fields" style="max-width:none">
        ...
        <label class="v2-field">${swap ? "Контракт стороны 1" : "Текущий поставщик (контракт)"}<select data-f="from" ${dis}>${contractOptions(x.from)}</select></label>
        ...
        <label class="v2-field">${swap ? "Контракт стороны 2 (только с этой маркой)" : "Новый поставщик (контракт)"}<select data-f="to" ${dis || (swap && (!x.from || !x.mark) ? "disabled" : "")}>${contractOptions(x.to, swap && !ro ? onlyB : null)}${swap && x.to && !contractById(x.to) ? "" : ""}</select>${x.sideBError ? `<small class="v2-auth-error">${esc(x.sideBError)}</small>` : ""}${swap && x.from && x.mark && !x.sideBCounts.size && !x.sideBError ? `<small class="v2-muted">Марка «${esc(x.mark)}» больше нигде на объекте к контрактам не привязана.</small>` : ""}</label>
