...
import { setImpersonationToken } from "./api.js";
...
export function mountUsersAccess(container, ctx) {
  ...
  // «Зайти под пользователем» (отладка чужих прав): сервер выдаёт отладочный сеанс, а ПЕРЕКЛЮЧАЕТСЯ сама эта
  // вкладка V2 (2026-09-22, не новая вкладка V1, как было раньше и как до сих пор делает сама V1) — задание
  // именно об этом: отладка прав не должна уводить администратора из V2. Токен кладётся в sessionStorage ПОД
  // ТЕМ ЖЕ КЛЮЧОМ, что уже использует V1 (api.js: setImpersonationToken, тот же приём и то же имя ключа, что
  // app/static/app.js), и вкладка перезагружается целиком: обёртка fetch в api.js подхватывает токен из
  // sessionStorage сама (и в оболочке V2, и в кадре сцены V1 рабочего места «Модель»/«МФР» — тот же
  // top-level browsing context, тот же origin, общий sessionStorage), права и меню пересчитываются обычным
  // путём входа (main.js: boot → /me → /me/permissions). requestLeave() — тот же сторож несохранённого, что и у
  // остальных переходов этого модуля: полная перезагрузка стёрла бы черновик карточки молча.
  async function startImpersonation(u, statusEl) {
    if (!(await requestLeave())) return;
    if (!(await showConfirmDialog(
      `Переключить эту вкладку в режим «от имени» пользователя «${u.display_name}»?\n\nВы увидите систему ровно так, как её видит этот человек — те же пункты меню, те же объекты, те же права. Всё, что будет `
      + "изменено, попадёт в журнал с отметкой, что это сделали вы от его имени, — и в историю статусов изделий тоже. Выйти из режима можно в любой момент кнопкой в красной полосе сверху; он и сам закроется через несколько часов.",
      { confirmLabel: "Переключить", multiline: true }))) return;
    statusEl.textContent = "Открываем режим…";
    try {
      ...
      <div class="v2-result">
        ...
        ${canWriteUsers && !isSelf
          ? (currentUser.impersonated_by
              ? `<p class="v2-note">Нельзя открыть ещё один отладочный режим, пока активен текущий (вложенность запрещена) — сначала вернитесь к своей учётной записи кнопкой в красной полосе сверху.</p>`
             ...
             <p class="v2-note">Эта вкладка переключится и будет работать так же, как у этого человека — те же пункты меню, те же объекты, те же права. Все действия там записываются на ваше имя. Выйти можно в любой момент кнопкой в красной полосе сверху.</p>`)
