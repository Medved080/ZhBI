...
export const api = {
  ...
  readPost: (path, body) => {
    // отчёты, разбор одной ячейки «Графика поставки» (тот же отчёт, отдельным запросом — app/report_delivery.py),
    ...
    // `/plan-data` — чтение геометрии и палитры основной модели; сервер использует POST для отбора объекта.
    if (!/^\/reports\/[a-z0-9-]+(\/cell)?$/.test(path) && !/^\/objects\/\d+\/(block-works\/bulk-preview|blocks\/work-types-settings\/preview|crane-zone-versions\/drafts\/\d+\/preview)$/.test(path) && path !== "/schedule-versions/deviation" && path !== "/plan-data") throw new Error(`readPost: «${path}» не разрешено как чтение — используйте post()`);
