...
import {
  ...
  scheduleInputsBodyProblem, scheduleCalcBodyProblem, defaultContractsMapBodyProblem,
} from "./picker-gate.js";
import { statusBatchBodyProblem, plannedBatchBodyProblem, contractSetBodyProblem, commentBodyProblem, historyEditBodyProblem, fieldsBodyProblem,
...
export const objectWorksSettingsProblem = (b) => isObj(b) && Array.isArray(b.work_type_ids) &&
  b.work_type_ids.every((id) => Number.isInteger(id) && id > 0) &&
  new Set(b.work_type_ids).size === b.work_type_ids.length && hasRev(b) ? null : "форма состава работ объекта";
export const objectWorkDatesProblem = (b) => isObj(b) && hasRev(b) &&
  ["plan_start", "plan_end", "forecast_start", "forecast_end"].every((key) => key in b && dateOrNull(b[key])) &&
  typeof b.note === "string" && b.note.length <= 4000 ? null : "форма сроков работы объекта";
...
// Привязка к осям — выпадающий список меток object_grids (как в V1); форма пересылает name+обе оси разом, потому что PATCH перезаписывает обе оси при каждом вызове (`app/blocks.py::update_section`) — правка одной оси без другой стёрла бы уже заданную.
...
export const POLICY = [
  ...
  { id: "subtype.delete", screen: "subtypes", action: "Подтипы: удалить по плану последствий (используемый — с заменой подтипа у изделий, как в V1)", method: "POST", path: re(`/dictionaries/subtype/${ID}/delete`), onlyKeys: ["replacements", "mode"], risk: "общая, необратимо, рабочие данные (переносит подтип у изделий)", allowed: true, proof: "живая проверка: удаление неиспользуемого; используемый — замена у изделий, SQL до/после, 403, audit-set" },
  ...
  // Построчные групповые режимы (перенос V1: таблица «изделие → значение»), ветка feature/v2-lines — те же принципы безопасности, что у element.status.batch/element.planned.batch выше:
  // ожидаемое состояние КАЖДОЙ строки, предпросмотр последствий без записи, подтверждение увиденных чисел, конфликт — 409 без частичных изменений, повтор идемпотентен. Серверные маршруты — `app/element_rows.py`.
  { id: "element.planned.rows", screen: "element-ops", action: "Схема: плановая дата поставки пачки ПО СТРОКАМ — у каждого изделия своя новая дата (или снятие), сверка прежней даты строки, предпросмотр последствий (задана/заменена/снята), всё или ничего", method: "POST", path: re(`/element-ops/planned-date-rows`), check: plannedRowsBodyProblem, risk: "рабочие данные", allowed: true, proof: "scripts/verify_element_rows.py (HTTP) и браузер на настоящем backend (Docs/v2-progress/lines.md)" },
  { id: "element.status.rows", screen: "element-ops", action: "Схема: смена статуса пачки с контрактом ПО СТРОКАМ — у каждого изделия свой контракт после операции (число, «без контракта» или прежний), предпросмотр последствий (снятие/замена/назначение контракта, страж остатка), всё или ничего; распределение (`POST /contracts/{id}/allocations`) не подменяет", method: "POST", path: re(`/element-ops/status-rows`), check: statusRowsBodyProblem, risk: "рабочие данные, история статусов (не отменяется), остатки контракта", allowed: true, proof: "scripts/verify_element_rows.py (HTTP) и браузер на настоящем backend (Docs/v2-progress/lines.md)" },
  ...
  { id: "object-work.settings", screen: "ws-mfr", action: "Состав неблочных работ текущего объекта", method: "PUT", path: re(`/objects/\\d+/object-works/settings`), onlyKeys: ["work_type_ids", "expected_rev"], check: objectWorksSettingsProblem, risk: "данные плана МФР", allowed: true, proof: "проверяется на копии БД; сервер валидирует объект, право, ревизию и мягкое снятие" },
  { id: "object-work.dates", screen: "ws-mfr", action: "План и сроки неблочной работы объекта", method: "PATCH", path: re(`/objects/\\d+/object-works/\\d+`), onlyKeys: ["plan_start", "plan_end", "forecast_start", "forecast_end", "note", "expected_rev"], check: objectWorkDatesProblem, risk: "данные плана МФР", allowed: true, proof: "проверяется на копии БД; сервер валидирует объект, право, ревизию и даты" },
  { id: "object-fact.create", screen: "ws-mfr", action: "Новый документ факта по объекту", method: "POST", path: re(`/objects/\\d+/object-fact-reports`), onlyKeys: ["report_date", "items"], check: factReportProblem, risk: "данные факта МФР", allowed: true, proof: "проверяется на копии БД; чужие работы и некорректные проценты отклоняются" },
  { id: "object-fact.update", screen: "ws-mfr", action: "Исправление документа факта объекта с отпечатком", method: "PUT", path: re(`/objects/\\d+/object-fact-reports/\\d+`), onlyKeys: ["report_date", "items", "expected_rev"], check: factReportUpdateProblem, risk: "данные факта МФР", allowed: true, proof: "проверяется на копии БД; 409 при устаревшем документе" },
  ...
  { id: "section.rename", screen: "blocks", action: "Секции: переименовать; привязка к осям — выпадающий список осей объекта (перенос из V1)", method: "PATCH", path: re(`/objects/\\d+/sections/\\d+`), onlyKeys: ["name", "axis_from", "axis_to"], check: sectionRenameProblem, risk: "структура объекта", allowed: true, proof: "браузер и HTTP: правка подписи и осей → SQL, 403" },
  ...
  { id: "block.boxes", screen: "blocks", action: "Геометрия блока: набор прямоугольников — перетаскиванием мышью (угол/ребро/тело) или числовыми полями, полный набор разом, с предпросмотром на плане", method: "PUT", path: re(`/objects/\\d+/blocks/\\d+/boxes`), onlyKeys: ["boxes"], check: blockBoxesProblem, risk: "геометрия объекта, влияет на автоматическое определение секции элементов", allowed: true, proof: "браузер и HTTP: запись мышью и числом → SQL (block_boxes), предупреждение о пересечении не блокирует сохранение, 403" },
  ...
  { id: "counterparty.delete", screen: "counterparties", action: "Контрагенты: удалить контрагента по плану последствий — заменить ссылки по отдельности, ИЛИ, если есть подчинённые, свернуть дубль (перенести договоры со всем содержимым к другой записи)", method: "POST", path: re(`/dictionaries/counterparty/\\d+/delete`), check: contractingDeleteBodyProblem, risk: "общая, необратимо", allowed: true, proof: "scripts/picker_verify/cp.mjs: предпросмотр, удаление, отказ при ссылках без замены, замена с переносом, полный откат; scripts/picker_verify/cp4.mjs: свёртка — перенос и слияние одноимённых, откат" },
  { id: "agreement.delete", screen: "counterparties", action: "Договоры: удалить договор по плану последствий — заменить или (свёртка дублей) перенести спецификации со всем содержимым к другому договору", method: "POST", path: re(`/dictionaries/agreement/\\d+/delete`), check: contractingDeleteBodyProblem, risk: "общая, необратимо", allowed: true, proof: "scripts/picker_verify/cp.mjs; scripts/picker_verify/cp4.mjs (свёртка)" },
  { id: "specification.delete", screen: "counterparties", action: "Спецификации: удалить спецификацию по плану последствий — заменить или (свёртка дублей) перенести контракты к другой спецификации", method: "POST", path: re(`/dictionaries/specification/\\d+/delete`), check: contractingDeleteBodyProblem, risk: "общая, необратимо", allowed: true, proof: "scripts/picker_verify/cp.mjs; scripts/picker_verify/cp4.mjs (свёртка)" },
  ...
  // Контракт по умолчанию по типу изделия (app/contracts.py): замена ВСЕЙ карты объекта — только по ожидаемой версии (query `expected_version`, 16 hex)
  { id: "default-contracts.set", screen: "contracts", action: "Контракт по умолчанию по типу изделия: заменить карту объекта целиком", method: "PUT", path: re("/contracts/default-map"), check: defaultContractsMapBodyProblem, risk: "общая для объекта (подставляется автоматически при первом уходе изделия со статуса «Запланирован»)", allowed: true, proof: "scripts/picker_verify/defmap.mjs: показ разницы, сохранение, конфликт версии, 403, обрыв ответа, SQL" },
  // График СМР (app/schedule_calc.py, app/schedule_versions.py): расчёт с предпросмотром (save:false, не пишет) и подтверждением (save:true, атомарно,
  // с отпечатком исходных данных, которые видел человек), исходные данные (темп/порядок/поток) целиком с отпечатком, разбор файла обработки (не пишет),
  // удаление версии (базовую — только администратор сервиса, серверная проверка).
  { id: "schedule.inputs.parse", screen: "schedule", action: "График СМР: прочитать файл обработки заказчика — разбор в форму, в базу не пишет", method: "POST", path: re("/schedule-calc/inputs/parse"), check: uploadCheck({ ext: ["xlsx"], fields: { object_id: isIntStr } }), risk: "данные не меняет (значения подставляются в форму)", allowed: true, proof: "scripts/picker_verify/sched.mjs: разбор, сопоставление со строками формы, отказ на неверный файл" },
  { id: "schedule.inputs.save", screen: "schedule", action: "График СМР: исходные данные расчёта (темп, порядок, поток) — замена целиком, с отпечатком того, что видел человек", method: "PUT", path: re("/schedule-calc/inputs"), check: scheduleInputsBodyProblem, risk: "данные расчёта графика", allowed: true, proof: "scripts/picker_verify/sched.mjs: сохранение и SQL, конфликт версии, 403, обрыв ответа" },
  { id: "schedule.calc", screen: "schedule", action: "График СМР: расчёт — предпросмотр (не пишет) и подтверждение (новая версия прогноза; отпечаток исходных данных, которые видел человек)", method: "POST", path: re("/schedule-calc"), check: scheduleCalcBodyProblem, risk: "рабочие данные: новая версия графика", allowed: true, proof: "scripts/picker_verify/sched.mjs: предпросмотр без записи, подтверждение и SQL, конфликт устаревших исходных данных, повтор безопасен, 403" },
  { id: "schedule.version.delete", screen: "schedule", action: "График СМР: удалить версию (базовую — только администратор сервиса)", method: "DELETE", path: re(`/schedule-versions/\\d+`), risk: "данные графика, необратимо", allowed: true, proof: "scripts/picker_verify/sched.mjs: удаление и SQL, 403 не-администратору на базовой, 404 повторно" },
  ...
  { id: "crane-zone.draft", screen: "zones", action: "Краны и стоянки: создать черновик редакции", method: "POST", path: re(`/objects/\\d+/crane-zone-versions/drafts`), check: noBodyProblem, risk: "черновик, без изменения рабочих зон", allowed: true, proof: "изолированная копия БД: атомарное создание и проверка прав" },
  { id: "crane-zone.edit", screen: "zones", action: "Краны и стоянки: сохранить целый черновик", method: "PATCH", path: re(`/objects/\\d+/crane-zone-versions/drafts/\\d+`), onlyKeys: ["edit_token", "zones", "overrides", "note"], check: (b) => (Number.isInteger(b?.edit_token) && b.edit_token > 0 && Array.isArray(b?.zones) && b.zones.length <= 2000 && isObj(b?.overrides) && typeof b?.note === "string" && b.note.length <= 2000 ? null : "неверный состав черновика"), risk: "черновик, без изменения рабочих зон", allowed: true, proof: "синтетический объект: валидация, конкуренция двух операторов, отказ без частичной записи" },
  { id: "crane-zone.publish", screen: "zones", action: "Краны и стоянки: опубликовать редакцию и атомарно сменить назначения", method: "POST", path: re(`/objects/\\d+/crane-zone-versions/drafts/\\d+/publish`), onlyKeys: ["edit_token", "effective_date"], check: (b) => (Number.isInteger(b?.edit_token) && b.edit_token > 0 && /^\d{4}-\d{2}-\d{2}$/.test(b?.effective_date || "") ? null : "неверная дата или версия черновика"), risk: "рабочие зоны и привязки изделий; публикация необратима, откат новой редакцией", allowed: true, proof: "синтетический объект: права, дата, один BEGIN IMMEDIATE, отказ/rollback при сбое; браузер на обезличенной копии" },
  ...
  { id: "users.impersonate", screen: "users-access", action: "Зайти под пользователем (отладочный сеанс; переключает ЭТУ вкладку V2 — 2026-09-22, было открытие V1 в новой вкладке)", method: "POST", path: re(`/users/\\d+/impersonate`), risk: "права, журнал на имя администратора", allowed: true, proof: "HTTP+браузер: токен, заголовок X-Impersonate-Token, отметка в журнале, 403, scripts/verify_access_impersonation.mjs" },
  ...
  { id: "revit.clear", screen: "revit-import", action: "Загрузка из Revit: отладочная очистка справочников объекта перед повторной загрузкой (применяется сразу, без сводки) — та же серверная операция, что у pdf.clear", method: "POST", path: re(`/objects/\\d+/clear-import-data`), check: clearImportDataProblem, risk: "необратимо через интерфейс (элементы модели Revit и/или секции/этажи и/или виды работ блоков); копия базы перед очисткой", allowed: true, proof: "настоящий backend: очистка по отмеченным группам (source: revit чистит разделы, кроме PDF), счётчики в ответе, отказ 403, журнал" },
