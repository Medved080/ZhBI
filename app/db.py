...
_COLUMN_MIGRATIONS = [
    ...
    # Изначально — недавние объекты пользователя для поповера в тулбаре V1;
    # функция убрана 2026-09-08 до того, как хоть строка кода её прочитала
    # или записала. 2026-09-22: столбец переиспользован под личные настройки
    # оболочки V2 (закреплённые объекты выбора объекта в шапке, состояние
    # левой навигации) — см. app/users.py::set_v2_shell_prefs,
    # app/auth.py::v2_shell_prefs_of. На ПОЛЬЗОВАТЕЛЯ, а не в localStorage, по
    # той же причине, что и last_object_id рядом: человек садится за другой
    # компьютер и должен увидеть те же настройки. Хранится текстом, как
    # menu_prefs: читается и пишется целиком, связей с чем-либо нет.
