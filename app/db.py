...
_COLUMN_MIGRATIONS = [
    ...
    # ------------------------------------------------------------------
    # Реквизиты объекта строительства из внутреннего реестра заказчика
    # (2026-09-08, импорт из Excel «Объекты на карте»): подразделение,
    # ответственные, ссылка на архив фото/видео. Только у objects — у
    # проекта (группы объектов площадки) своих таких данных нет, они
    # принадлежат конкретной стройке.
    #
    # Значения хранятся ТАК ЖЕ, как в источнике, включая буквальное
    # «Не требуется» у smu/smu_director: это не пропуск, а осмысленный факт
    # («этому объекту подразделение не назначается»), и NULL стёр бы разницу
    # между «не заполнено» и «заполнено явным отказом».
    ("objects", "smu", "TEXT"),
    ("objects", "smu_director", "TEXT"),
    # ДП/РП — ответственный по объекту (директор проекта / руководитель
    # проекта); в источнике формат ФИО не унифицирован, поле — как есть.
    ("objects", "responsible", "TEXT"),
    # Ссылка на папку с фото/видео объекта (обычно облачное хранилище).
    # Только ссылка, без скачивания: сервер за пределы сети не выходит
    # (см. Docs/DECISIONS.md), а вложение прикладывается человеком вручную
    # через уже существующий механизм — см. Docs/TZ.md §6.3.1 «Вложения».
    ("objects", "media_url", "TEXT"),
    # Аватарка — превью в дереве справочника (2026-09-08). Не отдельное
    # хранилище файла, а ссылка на уже существующее вложение объекта: две
    # параллельные системы загрузки/прав/удаления файлов ради одной
    # картинки того не стоят. SET NULL — удалили вложение, аватарка тихо
    # пропадает, а не превращается в битую ссылку.
    ("objects", "avatar_attachment_id", "INTEGER REFERENCES attachments(id) ON DELETE SET NULL"),
    # Дата начала СМР ИЗ РЕЕСТРА ЗАКАЗЧИКА (колонка «Старт СМР» в файле
    # «Объекты на карте»), строкой ГГГГ-ММ-ДД. Не путать с агрегатом
    # «Сроки СМР сводно» в карточке объекта — тот СЧИТАЕТСЯ по датам
    # элементов (`elements.project_smr_start_date`) и меняется сам; это
    ...
    # Этап 4 задания «Запланированная работа по блоку» (2026-09-10).
    # Построчный аудит факта: кто и когда правил ИМЕННО эту строку — раньше
    # подсказка истории показывала только автора/редактора ОТЧЁТА целиком
    # (work_fact_reports.created_by/updated_by), что при правке нескольких
    # операций одним отчётом не позволяло отличить, кто трогал какую.
    ("work_fact_items", "updated_at", "TEXT"),
    ("work_fact_items", "updated_by", "INTEGER REFERENCES users(id) ON DELETE SET NULL"),

    # Мягкое снятие операции с блока (решение пользователя В7) — «Настройки»
    # больше не удаляют ЗР безвозвратно, если на ней уже есть факт или
    # заданные сроки: строка помечается retired_at и пропадает из активного
    # плана/отбора, но остаётся адресуемой по id — история (факт, версии
    # прогноза) не привязана ни к чему повисшему. Тот же приём, что у
    # work_types.retired_at в справочнике видов работ.
    ("block_works", "retired_at", "TEXT"),

    # Поворот слоя внешней 3D-модели (2026-09-10, живой запрос
    # пользователя после первой версии — только сдвига X/Y оказалось
    # недостаточно). Таблица сама уже могла быть создана schema.sql БЕЗ
    # этой колонки на копиях, где функция обкатывалась раньше добавления
    # поворота.
    ("object_external_models", "rotation_deg", "REAL NOT NULL DEFAULT 0"),

    # Смещение по высоте (2026-09-11, живой запрос пользователя): по
