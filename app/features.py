...
FEATURES = [
    ...
    Feature("dict_smu", "Справочники", "СМУ",
            "Читать справочник может каждый (заполняет реквизиты объекта из выпадашки); "
            "заводить и переименовывать записи — только администратор сервиса.",
            ["GET /smu", "POST/PATCH /smu"], SCOPE_SERVICE, _все(READ)),
    Feature("dict_individuals", "Справочники", "Физлица",
            "Один справочник на обе роли объекта — «Директор СМУ» и «Ответственный (ДП/РП)».",
            ["GET /individuals", "POST/PATCH /individuals"], SCOPE_SERVICE, _все(READ)),
    Feature("address_load", "Справочники", "Загрузка адресного классификатора КЛАДР",
            "Разовая операция обслуживания: пишет на диск сервера сотни мегабайт и занимает "
            "минуты. Подсказки по адресу видны всем вошедшим и от этого раздела не зависят.",
            ["GET /address/regions-in-file", "POST /address/load, /address/unpack"],
    ...
    Feature("import_objects", "Загрузка данных файлом", "Загрузка справочника объектов из Excel",
            "Создаёт и правит проекты и объекты пачкой — тот же уровень риска, что и полный "
            "доступ к справочнику «Проекты и объекты», поэтому не выдан заводски никому.",
            ["POST /objects-import/analyze|apply"], SCOPE_SERVICE, _только_админ_сервиса()),
