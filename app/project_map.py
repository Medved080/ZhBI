...
# вырезку. Поэтому есть второй путь — брать карту и геокодирование прямо из
# OpenStreetMap.
#
# ОДИН выключатель на оба сервиса: тайлы карты и подсказка координат по
# адресу (Nominatim) — это один и тот же вопрос «разрешено ли браузеру
# ходить на сервисы OpenStreetMap», и заводить под него две настройки
# незачем. ВЫКЛЮЧЕН ПО УМОЛЧАНИЮ и включается администратором осознанно: это
# единственное место во всём сервисе, где браузер пользователя ходит на
# чужой адрес. Адреса вшиты в код, а не берутся из настройки: настройка с
...
# Геокодирование — определение координат ПО АДРЕСУ. Ни КЛАДР, ни ГАР
# координат не содержат (см. `app/kladr.py`), поэтому единственный способ
# заполнить их автоматически — публичный геокодер OpenStreetMap. Он же
# избавляет от ручной расстановки пина для каждой из сотен площадок.
GEOCODE_HOST = "https://nominatim.openstreetmap.org"
GEOCODE_URL = GEOCODE_HOST + "/search"

# Оба хоста разом — чтобы политика безопасности (app/main.py) не знала про
# них порознь и не разъехалась при добавлении третьего сервиса когда-нибудь.
ONLINE_HOSTS = (ONLINE_TILES_HOST, GEOCODE_HOST)
...
def _заголовок_подложки(путь: str) -> Optional[dict]:
    """Покрытие и уровни файла из заголовка PMTiles v3 — 127 байт, без
    сторонних библиотек. Нужны браузеру для правила совмещения нескольких
    файлов (`порядокФайлов()` в `map.js`): детальный файл рисуется поверх
    обзорного только с уровня, где у обзорного кончаются свои тайлы."""
    try:
        with open(путь, "rb") as f:
            h = f.read(127)
    except OSError:
...
def _версия_подложки(путь: str) -> str:
    """Версия файла для адреса: время изменения и размер. Файл заменили —
    адрес другой, и браузер не возьмёт из кэша старые заголовок и диапазоны
    (проверено 2026-09-22: после замены файла под тем же адресом все
    запросы шли из кэша со старым ETag, новый файл не был виден неделю)."""
    st = os.stat(путь)
...
def basemaps() -> list:
    ...
    for имя in имена:
        ...
        версия = _версия_подложки(путь)
        out.append({"name": имя, "size": os.path.getsize(путь),
                    "url": "/map/tiles/%s?v=%s" % (имя, версия), "version": версия,
                    "problem": беда,
                    "bbox": заголовок["bbox"] if заголовок else None,
                    "min_zoom": заголовок["min_zoom"] if заголовок else None,
...
def map_config(user: sqlite3.Row = Depends(require_service_feature("map", "read"))):
    ...
    return {
        ...
        "geocode_url": GEOCODE_URL if _online_tiles else None,
    }
...
def map_tiles(name: str, v: Optional[str] = None, user: sqlite3.Row = Depends(get_current_user)):
    ...
    Кэш — по версии в адресе (`?v=` из `/map/config`): совпала с файлом —
    ответ неизменен и кэшируется надолго; нет версии или она устарела
    (страница открыта до замены файла) — только с перепроверкой, чтобы
    браузер не склеил старые диапазоны с новым файлом.
    """
    путь = os.path.join(MAP_DIR, _safe_name(name))
    if not os.path.isfile(путь):
        raise HTTPException(status_code=404, detail="Файл карты не загружен")
    кэш = ("public, max-age=31536000, immutable" if v and v == _версия_подложки(путь)
           else "no-cache")
    return FileResponse(path=путь, media_type="application/octet-stream",
                        headers={"Cache-Control": кэш})
