...
def _fill_marks_catalog(conn) -> str:
    ...
    if не_разложено := conn.execute(
        ...
        хвост = ""
    # Источника ДВА, и в отчёте они названы порознь (2026-08-17). Раньше
    # печаталось только «из изделий», а вставка из позиций контрактов
    # считалась молча — и повторный прогон, заведший марку из позиции,
    # выглядел как «заведено: 0» при выросшем справочнике. На холостом
    # прогоне перед деплоем это прочиталось как нарушение идемпотентности,
    # хотя обработка отработала ровно как задумана: позиции контрактов,
    # заведённые ПОСЛЕ её первого выполнения, справочник догоняет при
    # повторе.
    return (f"записей справочника марок: {всего_записей} "
            f"(заведено этим проходом — из изделий: {из_изделий}, "
            f"из позиций контрактов: {из_контрактов}); "
...
def _fill_revit_sections_by_volume(conn: sqlite3.Connection) -> str:
    """Доопределить секцию по ОБЪЁМУ блока (`revit_sections.fill_by_volume`,
    2026-09-09) у уже загруженных элементов Revit — второй проход поверх
    `_fill_revit_sections`.

    Растровое голосование (`fill_missing`) строит зону только по элементам,
    у которых секция УЖЕ известна: если у секции нет вообще ни одного
    такого элемента (ручное добавление секции задним числом, либо секция
    целиком без параметра в модели), голосовать за её код некому, и
    растр не присвоит её ни разу, сколько раз ни запускай. Объём — не
    зависит от соседей: геометрия своя (`block_geometry.block_box`,
    прямая геометрия блока или, второй по приоритету, оси здания), и
    подбирает такие элементы с нуля."""
    from app import revit_sections
...
RELEASE_TASKS = [
    ...
    {
        "name": "2026-09-07-grant-map-read",
        # Версия — та, с которой карта выйдет; запись журнала версий под неё
        # ещё не согласована с пользователем (стоячая инструкция).
        "version": "0.69",
        "date": "2026-09-07",
        "title": "Открыть раздел «Карта проектов» существующим ролям",
        "why": "новый раздел прав по умолчанию не выдан никому "
               "(app/db._seed_object_roles: нет строки — значит «Нет»), а карта "
               "показывает ровно те объекты, которые роли и так доступны: без "
    ...
    {
        "name": "2026-09-09-revit-sections-by-volume",
        # Версия — та, с которой выйдет объёмное доопределение секции;
        # запись журнала версий под неё ещё не согласована с пользователем
        # (стоячая инструкция).
        "version": "0.80",
        "date": "2026-09-09",
        "title": "Доопределить секцию по объёму блока у элементов без секции",
        "why": "растровое доопределение (`_fill_revit_sections`) не может "
               "присвоить код секции, у которой ДО СИХ ПОР нет ни одного "
               "элемента с надёжной секцией — голосовать за неё некому; "
               "объём — геометрия самой секции, а не соседей, и подбирает "
    ...
    # «Обучение» пропустили в заходе выше (09-14): раздел появился раньше
    # остальных одиннадцати (2026-08-14, вместе с самой настройкой ролей) и
    # свою разовую раскладку уже получил тогда же — но ТОЛЬКО ролям,
    # существовавшим на тот момент (`2026-08-14-grant-training-read`). Роль,
    # заведённая позже (новая роль пуста — app/roles.py), инструкцию так и
    # не получила, и «Обучение» пропадало из «Действия» целиком (обоих
    # пунктов группы) — живой отчёт 2026-09-16.
    {
        "name": "2026-09-16-grant-training-baseline",
        "version": "0.85",
        "date": "2026-09-16",
        "title": "Открыть «Обучение» ролям, заведённым после 2026-08-14",
        "why": "новая роль заводится пустой, а разовая раскладка 2026-08-14 донастроила "
...
def backup_before_update() -> Optional[dict]:
    """Копия базы ПЕРЕД КАЖДЫМ стартом сервера — до миграций схемы и до
    ...
    До 2026-09-14 копия снималась, только если версия кода (верхняя запись
    `app/changelog.py`) отличалась от версии, записанной в базе — расчёт был
    не плодить копии на рестарт при падении и `docker compose up`. Оказалось,
    что это же завязывало бэкап на наличие записи в changelog: пуш в
    `develop`/`production` без изменения версии ехал вообще без копии.
    Решение пользователя: копия не должна зависеть от changelog — снимается
    на любой старт процесса безусловно, а не плодит копии сверх меры
    существующая ротация (`app/backups.py`, пять последних служебных копий):
    частые рестарты быстрее вытеснят старые копии, а не переполнят диск.

    Тихо ничего не делает, только если базы ещё нет (первый запуск вообще).
    """
    ...
    стало = code_version()
    комментарий = (
        f"перед обновлением {было or 'до v0.36'} → {стало}"
        if было != стало
        else f"перед стартом сервера (версия {стало or было or '?'})"
    )
    копия = backups.create_backup(kind=backups.KIND_BEFORE_UPDATE, comment=комментарий)
    if копия:
