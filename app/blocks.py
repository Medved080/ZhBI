...
def create_section(conn, object_id: int, code_input: str, name: str = None,
    ...
    и пропускает нормализацию целиком. Для остальных (в том числе ручного
    ввода из формы «Секции и этажи») — типовой числовой ввод («1»,
    «Секция 1», «01») приводится к «С0N», а код вне этого формата
    используется КАК ЕСТЬ, а не отклоняется: сама проверка формата
    (`revit_package.normalize_section`) защищает от МУСОРА автоматического
    Revit-импорта (docstring там же, «Автостоянка» из выгрузки) — не от
    осознанно заведённых секций вне формата «С0N» что руками через
    PDF-загрузку (2026-08-31, секция «Паркинг»), что теперь и через саму
    форму (2026-09-10, живой запрос пользователя, секция «Парковка 2»:
    «что за ограничение» — ограничение было лишним именно для ручного
    ввода)."""
    if trusted:
        code = code_input
    else:
        ...
        if not code:
            # Голое число без буквы/слова-префикса («1» само по себе) не
            # матчит общий regex `_SECTION_RE` (app/revit_package.py) — он
            # защищает автоматический Revit-импорт от мусора, трогать его
            # ради ручного ввода рискованно (шире регэксп — больше ложных
            # срабатываний на реальных Revit-параметрах). Отдельная,
            # локальная нормализация именно этого случая: подсказка формы и
            # текст прежней ошибки называли «1» валидным примером, и до
            # этой правки он единственный не проходил вовсе (2026-09-10,
            # найдено при разборе живого отчёта про секцию «Парковка 2»).
            голое = code_input.strip()
            code = "С%02d" % int(голое) if голое.isdigit() else голое
    if not code:
