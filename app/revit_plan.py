...
# (`_level_row`), собственное имя показывается как есть.
_DEFAULT_LEVEL_NAME_RE = re.compile(r"(?i)(этаж\s*-?\d+|-?\d+\s*этаж)")
...
def _level_row(row) -> dict:
    ...
    данные = dict(row)
    собственное = (данные.get("name") or "").strip()
    if данные["kind"] == "кровля":
        имя = "Кровля"
    elif данные["floor"] is None:
        имя = собственное or данные["key"]
    elif собственное and not _DEFAULT_LEVEL_NAME_RE.fullmatch(собственное):
        # Этаж с СОБСТВЕННЫМ именем — технический этаж, техпространство,
        # выход на кровлю — показывается им, а не «25 этаж» (2026-09-02,
        # живой запрос пользователя: «в секции 2 есть технический этаж, но
        # в отборах такого этажа нет» — был подписан «25 этаж»). Шаблонное
        # «Этаж 9»/«9 этаж» (Revit, `create_level` по умолчанию) — по-прежнему
        # словами по номеру, ниже.
        имя = собственное
    elif данные["floor"] < 0:
