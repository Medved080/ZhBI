...
def compute_floor_tier_elevations(element_records):
    """Границы ЭТАЖЕЙ (не путать с ярусами колонн выше) — уникальные
    отметки именно "Плита перекрытия", по возрастанию. У колонн одного
    физического этажа на реальных чертежах регулярно бывает НЕСКОЛЬКО
    разных отметок (например, ствол колонны состыкован из 2 кусков —
    "нижняя"/"нижний ярус") — считать границы этажей по ним дало бы
    лишний "этаж" там, где на самом деле один и тот же. У каждого этажа
    ровно одна плита перекрытия — потолок, поэтому именно её отметки и
    задают точные границы. См. Docs/backlog.md, временный расчёт этажа
    до прихода суффикса "_этаж N" в самих слоях (живой запрос
    пользователя, 2026-07-27)."""
    return sorted({
        r.elevation_mm for r in element_records
        if r.element_type == "Плита перекрытия" and r.elevation_mm is not None
    })


def assign_fallback_floors(element_records):
    """Заполняет record.floor (1..N) там, где он ещё None (т.е. слой
    элемента не нёс суффикс "_этаж N", см. layer_naming.parse_floor_token)
    — расчётом по отметке. НЕ то же самое правило strict_below, что у
    TIER_CAPPING_TYPES выше (там сравнение идёт с ярусами КОЛОНН, здесь —
    с границами ЭТАЖЕЙ, то есть отметками самих "Плита перекрытия" —
    отношение получается обратное):

    - "Плита перекрытия"/Ригель (сама отметка И ЕСТЬ граница этажа,
      которую они венчают СВЕРХУ) — этаж = порядковый номер этой границы
      снизу вверх, т.е. количество границ этажей <= собственной отметке
      (не меньше 1 — периметральный ригель ниже первой границы всё равно
      относится к этажу 1).
    - Колонна и всё остальное (отметка — это НИЗ яруса, элемент
      поднимается ОТ неё вверх, в пространство СЛЕДУЮЩЕГО этажа) — на
      единицу больше: количество границ этажей <= собственной отметке,
      плюс 1. Проверено на реальных данных: колонна "средняя верхний
      ярус" на отметке +15000 (там же, где "Плита перекрытия" +15000,
      потолок этажа 1) относится к этажу 2, а не 1 — при равных отметках
      колонна и плита оказываются на РАЗНЫХ этажах, потому что плита
      венчает этаж снизу неё, а колонна начинается и поднимается ВВЕРХ
      от этой же отметки.

    Ничего не делает, если границ этажей вообще нет (в файле нет ни
    одной "Плита перекрытия" — расчёт невозможен, floor остаётся None) —
    не подменяет собой реальный суффикс "_этаж N", когда он появится: тот
    проставляется в layer_naming.py РАНЬШЕ этого шага, эта функция
    трогает только записи, где floor всё ещё None."""
    tiers = compute_floor_tier_elevations(element_records)
    if not tiers:
        return
    for record in element_records:
        if record.floor is not None or record.elevation_mm is None:
            continue
        count = sum(1 for t in tiers if t <= record.elevation_mm)
        if record.element_type in TIER_CAPPING_TYPES:
            record.floor = max(1, count)
        else:
            record.floor = count + 1
