...
export const tests = [
  ...
  {
    id: "CP-W-05b", title: "Инциденты и производительность СОХРАНЯЮТСЯ: инцидент с датой и типом уходит в PATCH, без даты — отбрасывается, ноль в норме — отбрасывается (D-33)",
    async run(t) {
      const a = await openApp();
      await openContract(a, 4, "incidents");
