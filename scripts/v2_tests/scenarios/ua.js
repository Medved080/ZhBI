...
export const tests = [
  ...
  },
  {
    ...
    async run(t) {
      ...
      t.has(lastText(a), "Ничего не найдено по запросу «нетТакого»", "пустой результат объяснён запросом");
      t.ok(!lastText(a).includes("Нет доступа к проектам"), "поиск без совпадений не выдаётся за отсутствие доступа");
  ...
  },
  {
    ...
    async run(t) {
      ...
      // У пользователя с доступом поиск без совпадений — это «не найдено», а не «нет доступа».
