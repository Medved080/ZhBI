...
export const SCENES = {
  ...
  "read-record": { title: "Оболочка: экран чтения — запись (Адресный классификатор)", open: async (a) => { await waitFor(() => a.$(`${NAV}[data-section="address-classifier"]`), { what: "навигация" }); a.click(a.$(`${NAV}[data-section="address-classifier"]`)); await waitFor(() => a.$$("#rd-body dt").length >= 4, { what: "запись состояния" }); await a.settle(150); } },
  ...
  "shape-edit": { title: "Оболочка: «Форма маркеров» (изменена одна пара)", open: async (a) => { await waitFor(() => a.$(`${NAV}[data-section="marker-shapes"]`), { what: "навигация" }); a.click(a.$(`${NAV}[data-section="marker-shapes"]`)); await waitFor(() => a.$$("#se2-body select").length === 3, { what: "три пары" }); a.setValue(a.$$("#se2-body select")[1], "hexagon"); await a.settle(150); } },
  "shell-screen-form": { title: "Оболочка: экран V1-формы (каркас, «Плоская шахматка»)", open: async (a) => { await waitFor(() => a.$(`${NAV}[data-section="chess-flat"]`), { what: "навигация" }); a.click(a.$(`${NAV}[data-section="chess-flat"]`)); await waitFor(() => a.$(".v2-screen h2"), { what: "каркас экрана" }); await a.settle(150); } },
