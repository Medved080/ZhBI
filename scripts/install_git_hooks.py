...
Что ставится:
  ...
  * `pre-commit` — два стража: неопределённых имён
    (`scripts/check_undefined_names.py --staged`) — обращение к имени,
    которого нет, в коммит не проходит; и полноты реестра действий журнала
    (`scripts/check_activity_actions.py`) — новое событие без русской
    подписи и категории тоже не проходит.
