...
def copy_db(src, dst):
    ...
    """
    # База в режиме WAL (с 2026-08-14) открывается ТОЛЬКО ЧТЕНИЕМ лишь
    # тогда, когда рядом лежит её спутник `-shm`: иначе SQLite обязан его
    # создать и отвечает «unable to open database file». Рядом с работающим
    # сервером спутник есть, и там это незаметно, а на отдельном снимке —
    # нет, и проверка перед деплоем падала ровно в тот момент, когда нужна.
    #
    ...
    try:
        ...
        # он делает это лениво, и без запроса ошибка всплыла бы позже —
        # уже внутри backup(), мимо этого except.
        src_conn.execute("PRAGMA schema_version").fetchone()
    except sqlite3.OperationalError:
        сырой = dst + ".raw"
        for хвост in ("", "-wal", "-shm"):
            if os.path.exists(сырой + хвост):
                os.remove(сырой + хвост)
            if os.path.exists(src + хвост):
                shutil.copyfile(src + хвост, сырой + хвост)
        src_conn = sqlite3.connect(сырой)
        # Сводим журнал в саму базу, иначе копия унаследует ту же беду.
